...
import json
import torch
from GANLatentDiscovery.models.SNGAN.sn_gen_resnet import SN_RES_GEN_CONFIGS, make_resnet_generator
from GANLatentDiscovery.models.SNGAN.distribution import NormalDistribution
...
def load_model_from_state_dict(root_dir):
    args = Args(**json.load(open(os.path.join('GANLatentDiscovery/', root_dir, 'args.json'))))
    generator_model_path = os.path.join('GANLatentDiscovery/',root_dir, 'generator.pt')
